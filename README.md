...
How the decoder is put together (one feature file per value, one profile per model) is described in [Decoding by device profile](docs/decoding-by-device-profile.md). When something looks off, see [Troubleshooting](docs/troubleshooting.md). For contributors: [Adding a model or a value](docs/adding-a-model.md), [Evidence rules](docs/evidence-rules.md) (what ✅ 👁 ❓ 🔍 mean and how a value earns them), [Maintenance rules](docs/maintenance-rules.md) (invariants to keep when changing the code), and the per-model byte maps in [device analyses](docs/device%20analyzes/).
