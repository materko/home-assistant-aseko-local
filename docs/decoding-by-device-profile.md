...
- A **feature** is one target value: one field on `AsekoDevice`. pH is one feature, chlorine is another, `start2` is another. That is about 65 features for the frame-derived fields we have today.
