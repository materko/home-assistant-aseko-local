...
| `water_temperature_target` | `byte[55]` | °C            | Disabled on this device — see Open Item 3 |
...
‡ When antifreeze is ON, `byte[55]` drops from the normal heating setpoint
  ...
  `freeze_protection_enabled` is decoded from bit 7 (`0x80`), independent of
...
- **Antifreeze**: bit 7 (`0x80`) is decoded as `freeze_protection_enabled` (Issue #136).
...
| 7 | `byte[29]` per-pump bits for HOME (algicide, flocculant, cl, pH−) are unconfirmed. The masks in `ACTUATOR_MASKS[HOME]` are placeholders matching OXY/NET. Capturing frames with a single HOME pump running (e.g. algicide only) would pin down the per-pump bit. Until then, `algaecide_pump_running` and `flocculant_pump_running` may report incorrectly on HOME. |
...
| `test_decode_home_clf_real_frame` | Issue #110: real HOME CLF frame with `max_refill_time = 60` |
...
| `test_decode_home_algicide_pump_running` | Issue #115: `algaecide_pump_running` binary sensor is registered |
| `test_decode_home_floc_pump_running_independent` | HOME reports `flocculant_pump_running` correctly when only floc pump is installed |
...
| `test_filtration_pump_running_off_when_manual_override` | Issue #133: `byte[37]=0x35` forces `filtration_running=False` |
